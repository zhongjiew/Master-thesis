...
#使用方法, 在终端输入python blast_xml2csv.py -h 会输出详细参数介绍
#一般情况下在终端输出 ：python blast_xml2csv.py -i input filename -o output filename
#输出会默认生成两个文件，一个包含all hits，一个只包含best hit。而且输出名称一定要加上后缀 .csv
...
def parse_ncbi_xml_to_csv(infile, outfile, query, delim = ','):
    ...
    result = []
    header_list = ['query id',  'subject id', '% identity', '% query coverage', 'alignment length', 'mismatches', 'gap opens', 'q. start', 'q. end', 's. start', 's. end', 'evalue', 'bit score', 'subject description']
    ...
    for line in open(infile).readlines():
        if r_query_id.search(line) is not None:
            query_id =  line.split("<Iteration_query-ID>")[1].split("<")[0].strip('\n')
            if query != '':
                query_id = query_dict[query_id.split('_')[1]].strip('\n')
            ...
            query_cov = "%5.2f" %  (((float(hsp_align_len)/float(query_len))*100))
            ...
            var_list = [query_id, hit_id, percent_ident, query_cov, hsp_align_len, mismatch, hsp_gaps, hsp_query_from, hsp_query_to, hsp_hit_from, hsp_hit_to, hsp_evalue, hsp_bit_score, hit_def]
